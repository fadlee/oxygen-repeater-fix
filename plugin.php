...
class Oxygen_Repeater_Fix
{
	...
	function rewrite_css_uri( $buffer ) {
		$buffer = str_replace( '/wp-content/uploads/oxygen/css/', '/static/oxygen/css/', $buffer );
	...
	function watch_uri() {
		...
		if ( preg_match( '/static\/(?<path>oxygen\/css\/.*?\.css)/', $uri, $matches ) ) {
			$css_path = WP_CONTENT_DIR . '/uploads/' . $matches['path'];
